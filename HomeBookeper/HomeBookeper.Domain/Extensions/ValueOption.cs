...
public struct ValueOption<T> : IEquatable<ValueOption<T>> where T : struct
{
	...
	public Option<TResult> MapOption<TResult>(Func<T, Option<TResult>> map) where TResult : class
		=> _content.HasValue ? map(_content.Value) : Option<TResult>.None();

	public ValueOption<TResult> MapOptionValue<TResult>(Func<T, ValueOption<TResult>> map) where TResult : struct
